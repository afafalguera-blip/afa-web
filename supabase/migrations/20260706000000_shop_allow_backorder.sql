-- Allow orders even when a variant is out of stock (backorder / reserva).
-- Previously the function raised 'Insufficient stock for variant' and aborted the whole order.
-- Now the order is created and stock is decremented (may go negative), signalling a backorder.
-- Admin OrdersPage already highlights items where variant.stock < quantity.
