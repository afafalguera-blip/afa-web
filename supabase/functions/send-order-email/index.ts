...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch full order data including items and product names
    ...
    const itemsHtml = order.items.map((item: any) => `
      <tr>
        <td style="padding: 12px 8px; border-bottom: 1px solid #e2e8f0; color: #1e293b;">${item.variant.product.name}</td>
