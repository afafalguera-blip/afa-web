...
export function InscriptionsTable({
  ...
  if (loading) return <div className="p-8 text-center text-neutral-500">Carregant dades...</div>;

  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-neutral-100 bg-neutral-50/50 text-xs uppercase text-neutral-500 font-semibold">
        ...
        <tbody className="divide-y divide-neutral-100">
          {rows.map((row, idx) => (
            <tr key={`${row.inscription_id}-${idx}`} className="hover:bg-neutral-50 transition-colors group">
              <td className="px-6 py-4">
                <div className="font-medium text-neutral-900">{row.name} {row.surname}</div>
              ...
              <td className="px-6 py-4 text-neutral-600">{row.course}</td>
              <td className="px-6 py-4 text-neutral-600">
                <div className="flex flex-wrap gap-1">
                  {Array.isArray(row.activities) ? (row.activities as string[]).map((act, i) => (
                    <span key={i} className="text-[10px] bg-neutral-100 text-neutral-600 px-2 py-0.5 rounded">
                      {act}
                    </span>
                  )) : (
              ...
              <td className="px-6 py-4 text-sm">
                <div className="text-neutral-900 font-medium">{row.parent_phone}</div>
                <div className="text-neutral-500 text-xs">{row.parent_email}</div>
              </td>
              <td className="px-6 py-4">
                {row.afa_member ? (
                  ...
                  <span className="inline-flex items-center px-2 py-1 rounded-full text-[10px] font-bold uppercase bg-neutral-100 text-neutral-500">No soci</span>
                )}
              </td>
              <td className="px-6 py-4">
