    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 px-4 pt-4">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl mx-auto">
          La teva col·laboració ens permet seguir millorant l'experiència educativa de tots els infants de l'escola.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 border border-slate-100 dark:border-slate-700 shadow-sm relative overflow-hidden group">
            ...
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                <Info className="w-5 h-5 text-secondary" /> Per a què serveix?
            </h2>
            <div className="space-y-4 text-slate-600 dark:text-slate-400 leading-relaxed">
                <p>
                    L'AFA és una associació sense ànim de lucre formada per pares i mares. La quota anual és la nostra principal font de finançament per realitzar projectes i activitats.
                </p>
                <ul className="space-y-2">
                    <li className="flex items-start gap-2">
                        <CheckCircle className="w-4 h-4 text-green-500 mt-1 shrink-0" />
                        <span>Compvra de material pedagògic i lúdic per a l'escola.</span>
                    </li>
                    <li className="flex items-start gap-2">
                        <CheckCircle className="w-4 h-4 text-green-500 mt-1 shrink-0" />
                        <span>Organització de festes tradicionals (Castanyada, Carnestoltes, Fi de curs).</span>
                    </li>
                    <li className="flex items-start gap-2">
                        <CheckCircle className="w-4 h-4 text-green-500 mt-1 shrink-0" />
                        <span>Subvenció de xerrades i tallers per a famílies.</span>
                    </li>
                    <li className="flex items-start gap-2">
                        <CheckCircle className="w-4 h-4 text-green-500 mt-1 shrink-0" />
                        <span>Gestió de les activitats extraescolars i l'acollida.</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-secondary to-green-600 rounded-3xl p-8 text-white shadow-xl flex flex-col justify-center items-center text-center">
            <span className="text-secondary-foreground/80 font-bold uppercase tracking-widest text-sm mb-2">Quota Anual per Família</span>
            <div className="flex items-baseline gap-1 mb-4">
                <span className="text-6xl font-black">26</span>
                <span className="text-2xl font-bold">€</span>
            </div>
            <p className="text-white/80 text-sm leading-relaxed max-w-[240px]">
                Un únic pagament per curs escolar que beneficia a tots els teus fills/es.
            </p>
            <div className="mt-8 pt-6 border-t border-white/20 w-full">
                <p className="text-xs font-medium text-white/60 mb-1 italics">Referència de pagament</p>
                <p className="text-sm font-bold">CURS 2025-2026</p>
            </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/50 rounded-3xl p-8 border border-slate-200 dark:border-slate-800">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
            <Landmark className="w-6 h-6 text-primary" /> Informació per a la Transferència
        </h2>
        
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
                <div className="space-y-3">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">IBAN del AFA</label>
                ...
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl p-6 flex gap-4">
                    ...
                    <div className="space-y-2">
                        <p className="font-bold text-amber-900 dark:text-amber-200 text-sm uppercase tracking-wide">Molt Important!</p>
                        <p className="text-amber-800 dark:text-amber-300 text-sm leading-relaxed">
                            En el concepte de la transferència, si us plau:
                            <br />
                            <strong className="text-amber-900 dark:text-amber-100 uppercase">FEU CONSTAR EL NOM DEL VOSTRE FILL O FILLA I EL CURS.</strong>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-4">
                <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                    <p className="text-xs text-slate-400 font-bold uppercase mb-2">Entitat</p>
                    <p className="font-bold text-slate-700 dark:text-slate-200">Banco Sabadell</p>
                </div>
                <div className="p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                    <p className="text-xs text-slate-400 font-bold uppercase mb-2">Titular</p>
      ...
      <div className="text-center py-6 border-t border-slate-100 dark:border-slate-800">
          <p className="text-slate-400 text-xs font-medium italic">
              Un cop realitzada la transferència, recordeu guardar el comprovant per si fos necessari.
          </p>
      </div>
    </div>
  );
