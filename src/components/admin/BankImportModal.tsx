...
export function BankImportModal({ isOpen, onClose, onApplied }: Props) {
  ...
  const apply = async () => {
    ...
    try {
      ...
      setStage('done');
      toast.success(`${applied} rebuts marcats com a pagats`);
      onApplied();
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e);
      setError(message);
        ...
        {(stage === 'idle' || stage === 'loading') && (
          <div>
            <button
              ...
              className="w-full border-2 border-dashed border-neutral-300 rounded-xl p-10 flex flex-col items-center gap-3 text-neutral-600 hover:border-neutral-500 hover:text-neutral-900 transition-colors disabled:opacity-60"
            >
              {stage === 'loading'
                    ...
                    <div className="flex-1 min-w-0">
                      {row.candidatePayments.length > 0 ? (
                        ...
                        <select
                          ...
                          className="w-full text-sm px-2 py-1.5 border border-neutral-200 rounded-lg bg-white focus:ring-2 focus:ring-neutral-900/10 outline-none"
                        >
