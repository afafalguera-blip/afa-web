      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        {DAYS.map(day => {
          ...
          return (
            <div
              key={day.id}
              className={`flex flex-col rounded-3xl overflow-hidden transition-all relative z-10 ${hasSessions
                  ? 'bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700'
                  : 'bg-white/40 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 border-dashed opacity-60'
                }`}
            >
              {/* Day Header */}
              <div className={`p-4 text-center border-b ${hasSessions
                  ? 'border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800'
                  : 'border-slate-100 dark:border-slate-800'
                }`}>
                <h3 className={`font-black text-lg ${hasSessions ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'
                  }`}>
                  {day.label}
                </h3>
              </div>
                ...
                {hasSessions ? (
                  daySessions.map((item, idx) => (
                    <div
                      ...
                      className={`group relative overflow-hidden rounded-2xl p-4 cursor-pointer transition-all hover:scale-[1.02] hover:shadow-md border border-slate-100 dark:border-slate-700 ${item.activity.color?.replace('bg-', 'bg-opacity-10 bg-') || 'bg-blue-50 dark:bg-blue-900/20'
                        }`}
                    >
                      ...
                      <div className="pl-2">
                        ...
                        <div className="flex justify-between items-start mb-1">
                          <span className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md ${item.activity.color?.replace('bg-', 'text-') || 'text-blue-600'
